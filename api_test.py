...
#  ./api_test.py logspace=<logspace> search=<search terms> start=2023-01-18T11:34:00 \
...
url = "https://"+server+"/api/4/search/logspace/number_of_messages/\
%s?from=%d&to=%s&search_expression=%s" % (logspace, from_time, to_time, searchstring)
...
##############################################################
    offset =  n * 1000
    #print(offset)
    url = "https://"+server+"/api/4/search/logspace/filter/%s?from=%d&to=%s&search_expression=%s\
    &offset=%s&limit=%s" % (logspace, from_time, to_time, searchstring, offset,LIMIT)
